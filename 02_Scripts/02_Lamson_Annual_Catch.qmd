...
ggplot(data=LMS,aes(x=lbs_caught))+
  ...
  geom_vline(data=QS,aes(xintercept=q95),linetype="dashed")+
  geom_vline(data=QS,aes(xintercept=q99),linetype="solid")+
  #geom_text(data=QS,aes(label=q90,x=q90,y=Inf),hjust=1.2,vjust=2)+
  geom_text(data=QS,aes(label=q95,x=q95,y=Inf,),hjust=1.1,vjust=1.7)+
  geom_text(data=QS,aes(label=q99,x=q99,y=Inf),hjust=1,vjust=2.3)+
  facet_wrap(~sci_name,scales="free")+
  theme_bw(base_size=16)+
  labs(x="Catch per year (lb)",y="Number of fishers")

ggsave(last_plot(),file=fs::path(root_dir,"03_Outputs","Lamson_Sp_Lbs_Caught.png"),
       height=8,width=10,dpi=300)

# D7 totals
ggplot(data=LMS,aes(x=lbs_caught)) + geom_histogram(binwidth=25) +
  geom_vline(data=QS[sci_name=="Deep7"],aes(xintercept=q95),linetype="dashed")+
  geom_vline(data=QS[sci_name=="Deep7"],aes(xintercept=q99),linetype="solid")+
  xlim(0,2000)+
  theme_bw(base_size=15)+
  labs(x="Catch per year (lb)",y="Number of fishers")

ggsave(last_plot(),file=fs::path(root_dir,"03_Outputs","Lamson_D7_Lbs_Caught.png"),
       height=3,width=6)
