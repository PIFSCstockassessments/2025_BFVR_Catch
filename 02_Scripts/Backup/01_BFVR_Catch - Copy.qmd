---
title: "Deep7 non-commercial catch using the bottomfish vessel registry (BFVR)"
author: "Marc Nadon based on Jason Helyer's idea and scripts"
format: 
 html:
...
This script generates estimates of non-commercial catch for Deep7 in Hawaii based on the number of vessels registered in the Bottomfish Vessel Registry (BFVR) in a given year.

There are 3 major steps to do this:

1\) Create a cleaned-up BFVR list, with each entry being an individual *Name/Address* x *Year* instead of a *Vessel* x *Year*, as in the original list format. Since some fishers register multiple vessels, we need to pivot *vessel_ID* into multiple columns. Another major step is then to merge this fisher x year list with the CML fisher x year list, obtained from a list of permit transactions. This merged data set will provide a list (and count) of active BF fisher by year, with each fisher classified as BF-registered and/or CML-registered.

2\) Create annual distributions of species-specific mean-catch-per-vessel by county using a selection of fishers from the Fishing Reporting System (FRS) to serve as non-commercial proxies. This involves classifying all cml_no x year records as either "commercial" or "non-commercial", based on a series of filters : a) cml owner conducted a trip with a high catch in a given year, with per-trip max catch trigger points derived from MRIP "never-sell" interviews, and b) cml owner had a high catch per year, derived from Lamson (2007) mail surveys. These filters are applied at both the species level and all D7 together.

3\) Sample from these per-fisher (i.e. CML no) annual catch distributions proportionally to the number of non-commercial fisher per year per county, which was obtained in step 1.
...
-   **HFY1994_2023.rds** : raw Fishing Reporting System (FRS) commercial catch data.
...
# 1) Number of active BF fishers by year/county

This first major section merges the bottomfish registry (BF) and the CML transaction list (CL) in order to calculate the number of active non-commercial BF-registered vessels in a given year.

### Load the necessary data sets

There are 4 relevant data sets. To establish the BFVR list, we need the Bottomfish Registry (BF) list and the Commercial Marine License transaction list (CL). Extra licensee information for the CL is available in the "licensee" file. We also need a zipcode list (ZC) to classify addresses by county. Let's load and apply some basic filters to these data files.
...
BF <- fread(fs::path(root_dir,"01_Data","bfvr.csv")) %>% 
        select(cml_no.bf=cml_no,last_name.bf=last_name,
               ...
               city.bf=m_city,zipcode.bf=m_zipcode,
...
LC <- suppressWarnings( 
         ...
         select(cml_no.cl=license_number,first_name.cl=first_name,
                ...
                city.cl=residential_city,zipcode.cl=residential_zipcode)
...
## Bottomfish (BF) vessel registry data set preparation
...
# Delete records that were identified as duplicates or mistakes. These were checked individually and should be deleted.
...
# Select fields and rename appropriately. Note: middle initials were tried and there are too many mistakes and ommissions to be useful.
...
# Note: a lot of the duplicates are omitted "Sr" or "Jr" suffixes. Let's select the 2nd duplicated record, which will contain the suffixes (i.e. longer names).
...
BF <- BF %>% group_by(name.bf,phone.bf,year) %>% mutate(address.bf=
  if_else(is.na(address.bf)&!is.na(phone.bf),get_mode(address.bf),address.bf)) %>%    ungroup()
```
...
dups2 <- BF %>% ungroup() %>% 
            group_by(year,phone.bf,address.bf,city.bf,zipcode.bf) %>%
              summarize(N=n()) %>% select(-N) %>% filter(!is.na(phone.bf))

# Now we can create a unique list of Name x Year, and check which addresses are duplicated due to typos.  
...
BF <- BF %>% rename(address.bf=address.bf.x,
            ...
            select(-address.bf.y,-city.bf.y,-zipcode.bf.y)
```
...
dups2 <- BF %>% ungroup() %>% 
            group_by(year,cml_no.bf,address.bf,city.bf,zipcode.bf) %>%
...
# Check with cml_no
dups4 <- dups1 %>% filter(!is.na(cml_no.bf)) %>%  group_by(cml_no.bf,year) %>%          add_count() %>% filter(n()>1) %>% arrange(cml_no.bf,year) %>% as.data.table()
```
...
BF <- BF %>%
  select(name.bf,year,bf_registr,cml_no.bf,phone.bf,address.bf,city.bf,zipcode.bf,
         contains("vessel")) %>% as.data.table()
```

## Commercial Marine License (CL) data set preparation
...
```{r}
CL <- CL %>% mutate(ha_no=if_else(!is.na(ha_no),paste0("HA-",ha_no),NA),                          uscg=if_else(!is.na(uscg),paste0("USCG-",uscg),NA))
...
# First pivot the transaction and expiration year into a single column. 
CL <- CL %>%
   pivot_longer(cols=trans_year:exp_year,names_to="year_type",values_to="year") %>%       filter(!is.na(year)) %>% as.data.table() %>% arrange(name.cl,year)
...
CL <- CL %>% select(new_vessel_id.cl,cml_no.cl,year,
                    ...
                    city.cl,zipcode.cl,cml_registr.cl)
```

## Merge the Bottomfish Registry (BF) and the Commercial Fishing Licence (CL) lists together
...
# Note: We see some cml_no.bf that don't match the CL list. Those are mainly fishers stating "longliner/albacore" as their primary gear, and are likely recreationally bottomfishing on the side.

# To resolve ommitted suffix or other errors, let's simply select the longest name.
full.names <- full.names %>% mutate(
               name=if_else(nchar(name.bf)>=nchar(name.cl),name.bf,name.cl)) %>%                   rename(cml_no=cml_no.bf) %>%  select(name,cml_no)
...
D1 <- D1 %>% 
       mutate(year=if_else(is.na(name),year.cl,year),
              ...
              zipcode=
  if_else((zipcode<96701|zipcode>96898)&!is.na(zipcode.cl),zipcode.cl,zipcode))
```
...
D2 <- D1 %>% left_join(ZC,join_by(zipcode==zipcode.zc)) %>% 
             select(name,year,bf_registr,cml_registr,
                    cml_no,address,city,zipcode,county,island,
...
D2 <- D2 %>% mutate(
     ...
     # Note: below is only necessary if some fishers had 4 vessels registered.          #   vessel_4a=case_when(
...
```{r}
D2.check1 <- D2 %>% filter(!is.na(cml_no)) %>%  group_by(cml_no,year) %>%                                    add_count() %>% filter(n()>1)
```
...
# Do the same using thing using name.
...
writexl::write_xlsx(D2,fs::path(root_dir,"03_Outputs","Hawaii Fishers.xlsx"))
```

Now, for the out-of-state addresses, calculate prop. of BF by county and assign counties proportionally. If we simply filtered these addresses out, we would get a wrong BF count.
...
```{r}
D3 <- D2 %>% filter(cml_registr=="N"&bf_registr=="Y"&year<2025) %>% 
         group_by(year,county) %>% summarize(n_nc_bf_fishers=n())

ggplot(data=D3,aes(x=year,y=n_nc_bf_fishers,fill=county))+
      labs(x="Year",y="Non-commercial BF fishers (count)")+
      geom_bar(stat="identity",position="stack")+
      theme_classic(base_size = 14)

ggsave(last_plot(),file=fs::path(root_dir,"03_Outputs","NC_BF_count.png"),
       width=8,height=4)
```
...
```{r}
D4 <- D3 %>% pivot_wider(names_from=county,values_from=n_nc_bf_fishers) %>%
               mutate(total=rowSums(across(where(is.numeric))))

D4 %>% as.data.frame() %>% flextable()

write_csv(D4,file=fs::path(root_dir,"03_Outputs","NC_BF_count.csv"))
```

# 2) Annual catch per vessel

This second major section uses the Fishing Reporting System (FRS) commercial catch data, applies filters to trip and annual catches in order to obtain a non-commercial proxy catch data set. From this subset of fishers, we can obtain annual Deep7 NC catch distributions grouped by county.

## Enter the limits that define non-commercial proxies in the FRS

These limits are derived from the MRIP fishing trip interviews (2004-2022) and represent the 99th percentile of catches, when the catches are not zero.
...
We need the FRS catch reports, in addition to the fisher list from the first section.
...
Join catch data with fisher information from step 1.

```{r}
F1 <- F1 %>% left_join(D2,by=join_by(cml_no.fs==cml_no,year))

cat("A small number of records didn't match: ",nrow(unique(F1[is.na(county)])))

F1 <- F1 %>% filter(!is.na(county))

# NOTE: there are about 50 cml_no with no county info. Checking transaction list, these are CML that didn't have a vessel_id listed... and said Captain==N, but still reported D7 ("no vessel" cmls  were filtered early on in Step 1, thus the lack of match). What to do with these?
```
...
Classify all CMLs as either "commercial" (C) or "non-commercial" (NC), based on the per-trip limits defined previously.
...
F2 %>% filter(d7!=0&bf_registr=="Y"&year==2023) %>% ggplot(aes(x=d7))+
          geom_histogram(breaks=c(seq(0,500,by=10),max(F3$d7)),
                         position="identity")+
...
Let's sum the trip catch by *year* and *cml_no*, and merge the fisher information from the step above with this data set.

```{r}
F3 <- F2 %>% group_by(year,cml_no.fs,bf_registr,cml_registr,county) %>%                         summarize_if(is.numeric,sum)
...
An important step is to decide which fishers to remove from this data set. One filter we discussed was to remove fishers that did not catch a single D7 during a year. Another discussion is to just keep fishers registered on the BFVR.
...
Important step: decide which filters to apply. Let's start with keep fishers D7\>0 and BF registered only.
...
#NOTE: the difference between these two filters appear to be about 29,000 lb vs 30,000 lb yearly NC D7 average catch. Not as big of a deal as originally thought.
```
...
# 3) Generate annual non-commercial catch distribution

Now that we have 1) the number of active non-commercial BF fisher by year, and 2) a proxy data set of annual catch per "non-commercial" fisher, we can sample "n" times from that annual catch data set, where "n" is the number of non-comm. BF fishers in a specific *year* and *county*. This will generate a final distribution of non-commercial catches by year.

```{r}
# Add the # of NC vessels by year x County to the catch data
F4 <- F4 %>% left_join(D3,by=join_by(year,county)) %>% 
                relocate(n_nc_bf_fishers,.after=county)
...
for(i in 1:1000){
 aSample <- F4 %>% group_by(year,county) %>% 
                    sample_n(n_nc_bf_fishers[1],replace=T) %>% 
