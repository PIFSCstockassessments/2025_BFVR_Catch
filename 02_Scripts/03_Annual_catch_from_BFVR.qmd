...
This script creates annual distributions of species-specific
mean-catch-per-vessel by county using a selection of fishers from the
Fishing Reporting System (FRS) to serve as non-commercial proxies. This
involves classifying all cml_no x year records as either "commercial" or
"non-commercial", based on a series of filters : a) cml owner conducted
a trip with a high catch in a given year, with per-trip max catch
trigger points derived from MRIP "never-sell" interviews (see script
03_MRIP_interviews_catch.qmd), and b) cml owner had a high catch per
year, derived from Lamson (2007) mail surveys. These filters are applied
at both the species level and all D7 together.

The script does this by 1) loading a list of bottomfish fishers and
establishing the number of active fishers in a given year, 2) loading
catch reports contained in the Fisher Reporting System (FRS), 3)
selecting a subset of fishers from the FRS (i.e. non-commercial fisher
proxies) , and 4) sampling annual catches by county from these fishers.
The end product are distributions of non-commercial Deep 7 catches in
Hawaii, by year. Note that the fisher list and fisher count files come
from the Hawaii_Fisher_List repository that handles creating list of
fishers for this and other projects.
...
These limits are derived from the MRIP fishing trip interviews
(2004-2022) and represent the 99th percentile of catches, when the
catches are not zero.
...
```{r}
# Load fisher list from Hawaii_Fisher_List repository found on NOAA-PIFSC
FL <- read_xlsx(fs::path(root_dir,"01_Data","Hawaii Registered Fishers.xlsx"))

# Load fisher counts from Hawaii_Fisher_List repository.
FC <- fread(fs::path(root_dir,"01_Data","Fisher_counts.csv")) %>% 
...
Now, let's join catch data with fisher information from the Hawaii
Registered Fisher list.
...
[NOTE: there are about 50 cml_no with no county info. Checking
transaction list, these are CML that didn't have a vessel_id listed...
and said Captain==N, but still reported D7 ("no vessel" cmls were
filtered early on in Step 1, thus the lack of match). What to do with
these?]{.underline}
...
Save this Fisher trip information for the Shinny app used to explore the
different options available to generate non-commercial catch.

```{r}
write_csv(F2,fs::path(root_dir,"03_Outputs","FRS_trips.csv"))
```

Classify all CMLs as either "commercial" (C) or "non-commercial" (NC),
based on the per-trip limits defined previously.
...
Quick check on the distribution of trip catches for all BF + caught a D7
fishers.
...
Let's sum the trip catch by *year* and *cml_no*, and merge the fisher
information from the step above with this data set.
...
An important step is to decide which fishers to remove from this data
set. One filter we discussed was to remove fishers that did not catch a
single D7 during a year. Another discussion is to just keep fishers
registered on the BFVR.
...
Important step: decide which filters to apply. Let's start with keep
fishers D7\>0 and BF registered only.
...
NOTE: the difference between these two filters appear to be about 29,000
lb vs 30,000 lb yearly in average catch. Not as big of a deal as
originally thought.

Now let's only keep the "non-commercial" fisher proxies and clean up the
data.
...
Now that we have 1) the number of active non-commercial BF fisher by
year, and 2) a proxy data set of annual catch per "non-commercial"
fisher, we can sample "n" times from that annual catch data set, where
"n" is the number of non-comm. BF fishers in a specific *year* and
*county*. This will generate a final distribution of non-commercial
catches by year.
