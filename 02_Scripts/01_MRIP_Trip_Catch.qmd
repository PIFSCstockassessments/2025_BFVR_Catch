...
ggplot(data=ITS,aes(x=lbs_caught))+
  ...
  geom_vline(data=FS,aes(xintercept=q95),linetype="dashed")+
  geom_vline(data=FS,aes(xintercept=q99),linetype="solid")+
  #geom_text(data=FS,aes(label=q90,x=q90,y=Inf),hjust=1.2,vjust=2)+
  geom_text(data=FS,aes(label=q95,x=q95,y=Inf,),hjust=1.1,vjust=1.7)+
  geom_text(data=FS,aes(label=q99,x=q99,y=Inf),hjust=1,vjust=2.3)+
  facet_wrap(~sci_name,scales="free")+
  theme_bw(base_size=16)+
  labs(x="Catch per trip (lb)",y="Number of trips")
...
ggplot(data=IT.D7,aes(x=lbs_caught)) + geom_histogram(binwidth=5) +
  geom_vline(data=FS[sci_name=="Deep7"],aes(xintercept=q95),linetype="dashed")+
  geom_vline(data=FS[sci_name=="Deep7"],aes(xintercept=q99),linetype="solid")+
  xlim(0,500)+
