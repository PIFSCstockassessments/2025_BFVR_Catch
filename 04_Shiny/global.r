...
QT_trip <- fread(file.path("04_Shiny", "data", "mrip_quantiles.csv")) %>%
        pivot_longer(cols = q90:max, names_to="quantiles", values_to="value")
QT_annual <- fread(file.path("04_Shiny","data", "lamson_quantiles.csv")) %>%
        pivot_longer(cols = q90:max, names_to="quantiles", values_to="value")

# Get species codes and names for plots
SP.id <- QT_trip[,1:3]

FC <- fread(file.path("04_Shiny","data", "Fisher_counts.csv")) %>%
        filter(cml_registr == "N")

F2 <- fread(file.path("04_Shiny","data", "FRS_trips_annon.csv"))

# Read in CML catches and tidy
cml <- fread(file.path("04_Shiny","data", "CML_catches.csv"))
...
TC <- fread(file.path("04_Shiny","data", "Total_catches_2024.csv"))
